...
contract saveAMillion {
    ...
    function getUserInfo(
        ...
        external
        view
        returns (uint stakingAmount, uint tickets, uint rewards, bool isWinner)
    {
    ...
    function randomNumber(uint preSeed) internal view returns (uint) {
        ...
        bytes32 seed = keccak256(
            abi.encodePacked(block.difficulty, block.timestamp, uint(preSeed))
        );
    ...
    function issueReward(uint winningNumber) external {
        require(
            existingTicketNumber[winningNumber],
            "no user has winning ticket number"
        );
